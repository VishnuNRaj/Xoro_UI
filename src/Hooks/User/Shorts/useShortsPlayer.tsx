...
export default function useShortsPlayer ({ video, shorts, id, getMoreShorts }: UseShortsPlayerProps) {
  ...
  const handleSubscribe = () => {
    if (video && video.channel) {
      socket?.emit("subscribeChannel", video.channel._id, video.channel._id);
      setSubs(prev => prev + 1);
      setSubscribe(true);
    }
  };
  ...
  const handleUnsubscribe = () => {
    if (video && video.channel) {
      socket?.emit("unsubscribeChannel", video.channel._id, video.channel._id);
      setSubs(prev => prev -
