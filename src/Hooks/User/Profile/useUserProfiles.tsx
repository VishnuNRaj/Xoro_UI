...
import { followUser, unfollowUser,getProfile } from "@/Store/UserStore/ProfileManagement/ProfileSlice"
...
export default function useUserProfiles() {
    ...
    function FollowUser() {
        ...
        if (token) dispatch(followUser({ token, UserId: userData?._id as string })).then((state: any) => {
    ...
    function UnfollowUser() {
        const token: string | undefined = getCookie('token');
        if (token) dispatch(unfollowUser({ token, UserId: userData?._id as string })).then((() => {
            setConnection({ ...Connection, Follower: false })
        }))
    }
    return {
        type, setType, Connection, loading, loadingProfile, user, userData, post, socket, navigate, FollowUser, UnfollowUser
    }
}