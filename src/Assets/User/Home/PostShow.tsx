...
const PostShow: React.FC = () => {
    ...
    const { auth, dispatch, navigate, Post } = useEssentials()
        ...
        <div className='grid grid-cols-1 items-center justify-center space-y-8 w-full h-auto'>
            ...
            {post && (
                <div className="animate-slideInFromLeft p-4 flex h-full items-center justify-center">
                    {dialog && width <= 768 && <Dialog className='w-full' open={dialog} placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined} handler={() => setDialog(false)}><CommentComponent PostId={post._id} comments={comments} setComments={setComments} /></Dialog>}
                    <Toaster richColors position='top-right' duration={1500} />
                    <div className="bg-white border float-left rounded-lg w-full flex-shrink-0 md:min-w-[400px] max-w-md">
                        ...
                        {post.Caption.length > 0 && <div className={`w-full ${count.like !== 0 ? "mb-0" : "mb-2"} font-semibold mx-4 flex items-center flex-shrink-0 text-sm text-black`}>
                            <h1>{post.Caption}</h1>
                        </div>}
                    ...
                    {width > 768 && (
                        <div className='w-[600px] rounded-lg bg-slate-800 ml-10 flex-shrink-0 border-2 float-left'>
