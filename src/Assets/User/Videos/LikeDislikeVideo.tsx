...
import { MessageCircle, ThumbsUp, ThumbsDown, Share2, Facebook, Twitter } from 'lucide-react';
...
export default function LikeDislikeVideo({ post, dialog, setDialog, comment, width }: LikeDislikeVideoProps) {
  ...
  const getShareUrl = () => {
    ...
    return `${baseUrl}/videos/${post.VideoLink}`;
  };
  ...
  const handleShare = (platform?: string) => {
    ...
    switch (platform) {
      case 'facebook':
        ...
        break;
      case 'twitter':
        shareUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(videoUrl)}&text=${encodeURIComponent(post.Caption)}`;
        break;
      default:
    ...
    <div className="flex flex-wrap items-center justify-start gap-2 mt-2 sm:mt-0 sm:justify-end">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              size="sm"
              onClick={handleLike}
              className={`flex items-center hover:bg-transparent space-x-1 ${like ? 'text-secondary' : 'dark:text-white'}`}
            >
              <ThumbsUp className={`h-5 w-5 ${like ? "fill-current" : ""}`} />
              <span>{count.like > 0 && (count.like > 1000 ? `${Math.floor(count.like / 1000)}K` : count.like)}</span>
            </Button>
          </TooltipTrigger>
          <TooltipContent>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className={`flex items-center hover:bg-transparent space-x-1 ${dislike ? 'text-secondary' : 'dark:text-white'}`}
            >
              <ThumbsDown className={`h-5 w-5 ${dislike ? "fill-current" : ""}`} />
            </Button>
          </TooltipTrigger>
          <TooltipContent>
      ...
      <Dialog open={isShareOpen} onOpenChange={setIsShareOpen}>
        <DialogTrigger asChild>
          <Button variant="ghost" size="sm" className="flex items-center space-x-1 bg-primary-dark hover:bg-primary-light text-white hover:text-foreground">
          ...
          <div className="grid gap-4 py-4">
            <div className="flex items-center space-x-2">
              <Input
                ...
                className="flex-1"
              />
              <Button onClick={() => handleShare()} className="text-white">Copy</Button>
            </div>
