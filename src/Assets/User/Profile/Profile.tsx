    ...
    >
      ...
      <Card className="mx-auto rounded-none sm:rounded-lg overflow-hidden shadow-lg bg-blue-light dark:text-white dark:bg-background">
        <div className="relative h-48 sm:h-64 md:h-80">
          ...
          <div className="absolute top-4 right-4 flex space-x-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="dark:text-white gap-2 flex flex-col bg-blue-light dark:bg-background"
              >
                <DropdownMenuItem
                  className="font-semibold"
                  onSelect={() => bannerRef?.current?.click()}
                >
                ...
                <DropdownMenuItem
                  className="font-semibold"
                  onSelect={() => setIsShareOpen(!isShareOpen)}
                >
        ...
        <div className="relative px-4 sm:px-6 pb-4">
          <div className="flex flex-col sm:flex-row items-center sm:items-end -mt-20 sm:-mt-24 mb-4 sm:mb-0">
            <div className="relative">
              ...
              <Button
                variant="secondary"
                size="icon"
                className="absolute bottom-0 left-0 text-white rounded-full"
          ...
          <div className="w-full flex justify-center items-center text-center border-border dark:border-border-dark pt-4">
            <div className="flex items-center max-w-xl justify-between w-full mt-6 pt-4 border-t text-center">
              ...
              >
                <div>
                  <p className="text-xl font-bold">
                    {connections?.follow?.length || "5.6K"}
                  </p>
                  <p className="text-muted-foreground dark:text-muted-foreground-dark">
                    Followers
                  </p>
              ...
              >
                <div>
                  <p className="text-xl font-bold">
                    {connections?.following?.length || "789"}
                  </p>
                  <p className="text-muted-foreground dark:text-muted-foreground-dark">
                    Following
                  </p>
          ...
          <div className="mt-6">{type === "Images" && <ImgComponent />}</div>
        </div>
      </Card>
      <input
        ref={bannerRef}
        id="banner-upload"
        onChange={(e) => handleBanner(e, Profile, setBanner, user, dispatch)}
        type="file"
        className="hidden"
      />
      <input
        ref={profileRef}
        id="profile-upload"
        onChange={(e) => handleImages(e, Profile, setProfile, user, dispatch)}
        type="file"
        className="hidden"
      />
    </div>
