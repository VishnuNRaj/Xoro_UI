        ...
        <div className="flex items-center space-x-2">
          <Sheet>
            ...
            <SheetContent
              title=""
              side="left"
              className="w-[300px] sm:w-[400px] bg-blue-light dark:bg-background"
            >
              <nav className="flex flex-col h-full">
                <div className="flex items-center justify-between p-4 border-b border-border">
                  <h2 className="text-lg font-semibold flex gap-4 items-center justify-center dark:text-white">
                    <img
                      src="/Logo.png"
                      className="w-10 h-10 bg-transparent bg-blend-normal"
                      alt=""
                    />
                    Welcome to xoro
                  </h2>
                  ...
                  <div className="space-y-2">
                    {buttonsArray.map((button, index) => (
                      <SheetTrigger
                        asChild
                        key={index}
                        className="duration-400 animate-popup"
                      >
                        <Button
                          key={index}
                          variant="ghost"
                          className="w-full hover:bg-blue-300 dark:hover:bg-blue-dark flex gap-4 dark:text-white justify-start"
                          asChild
                        >
                ...
                <div className="p-4 border-t border-border">
                  {auth && auth?.user && (
                    <Button
                      onClick={logout}
                      className="w-full bg-destructive text-destructive-foreground hover:bg-secondary dark:text-white"
                    >
                      Logout
                    </Button>
                  )}
          ...
          {!auth || !auth?.user ? (
            ...
            <>
              ...
              <div
                ...
                className="h-9 rounded-full cursor-pointer bg-primary flex items-center gap-2 px-3 justify-center"
              >
